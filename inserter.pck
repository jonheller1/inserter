...
begin
	...
	--Check P_COMMIT_STYLE.
	if p_commit_style in (commit_style_at_end, commit_style_none, commit_style_per_batch) then
		null;
	else
		raise_application_error(-20000, 'P_COMMIT_STYLE must be set to either COMMIT_STYLE_AT_END, COMMIT_STYLE_NONE, or COMMIT_STYLE_PER_BATCH..');
	end if;

	--Check P_ESCAPE_STYLE.
	...
	--Check P_COLUMN_LIST.
	if p_column_list in (COLUMN_LIST_DERIVED_FROM_SQL, COLUMN_LIST_DERIVED_FROM_TABLE, COLUMN_LIST_NONE) then
		null;
	else
		raise_application_error(-20000, 'P_COLUMN_LIST must be set to either COLUMN_LIST_DERIVED_FROM_SQL, COLUMN_LIST_DERIVED_FROM_TABLE, or COLUMN_LIST_NONE.');
	end if;
...
begin
	...
	p_text := replace(replace(replace(replace(replace(replace(p_text
		,'#ROWCOUNT#', p_rowcount)
		,'#ROW_OR_ROWS#', v_row_or_rows)
		,'#USER#', user)
		,'#DATE#', to_char(sysdate, 'YYYY-MM-DD HH24:MI:SS'))
		,'#TABLE#', p_table_name)
		,'#SET_DEFINE_OFF#', chr(10)||'-- Prevent ampersands from being treated as substitution variables.'||chr(10)||'set define off;'
		);
